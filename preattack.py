...
class PreAttack:
    ...
    def extract_harm_target(self, org_query):
        ...
        for _ in range(5):
            try:
                ...
                print(f"Extract harm target response of {self.model_name}", res)  # Debugging output
                data = parse_json(res)
    ...
    def get_actors(self, harm_target):
        ...
        for _ in range(3):
            try:
                ...
                #print("RAW RESPONSE IN GET ACTORS", resp)
                data = parse_json(resp)
                #print("DATA AFTER GET ACTORS: ", data)
    ...
    def infer(self, num=-1):
        ...
        file_path = os.path.join('pre_attack_result', f'queries_for_{self.model_name.split("/")[-1].replace(".", "-")}_{num}_{datetime.now().strftime("%Y-%m-%d_%H-%M-%S")}.json')
